...
_PLATFORMS = [
    ...
    {
        "key": "agentphone",
        "label": "AgentPhone (voice calls)",
        "emoji": "📞",
        "token_var": "AGENTPHONE_API_KEY",
        "setup_instructions": [
            "1. Sign up at https://agentphone.to and create an agent to get your Agent ID",
            "2. In the AgentPhone dashboard, generate an API key",
            "3. Configure a webhook for the agent pointing at your Hermes host:",
            "   POST https://<your-public-host>:8646/agentphone/webhook",
            "   (port is configurable via AGENTPHONE_PORT)",
            "4. Save the webhook's signing secret (starts with whsec_) from the dashboard",
            "5. Decide which phone numbers the agent is allowed to call and which",
            "   inbound callers you trust — add them to the allowlist below",
            "6. Inbound calls from numbers NOT on the allowlist (and not the agent's",
            "   own number) will be rejected with HTTP 403",
            "7. Outbound calls via send_message/cron REQUIRE a call 'intent' and",
            "   'context_brief' so the agent on the call stays strictly on-topic",
        ],
        "vars": [
            {"name": "AGENTPHONE_API_KEY", "prompt": "AgentPhone API key", "password": True,
             "help": "The API key from your AgentPhone dashboard."},
            {"name": "AGENTPHONE_AGENT_ID", "prompt": "Agent ID (agt_...)", "password": False,
             "help": "The ID of the AgentPhone agent that will place/receive calls."},
            {"name": "AGENTPHONE_AGENT_PHONENUMBER", "prompt": "Agent's own phone number (E.164, e.g. +15551234567)", "password": False,
             "help": "The phone number assigned to your AgentPhone agent."},
            {"name": "AGENTPHONE_ALLOWED_INBOUND_NUMBERS", "prompt": "Allowed inbound phone numbers (comma-separated E.164)", "password": False,
             "is_allowlist": True,
             "help": "Inbound callers allowlist. Only these numbers (plus the agent's own number) can reach the agent via webhook. Outbound calls are unrestricted."},
            {"name": "AGENTPHONE_WEBHOOK_SECRET", "prompt": "Webhook signing secret (whsec_...)", "password": True,
             "help": "Copy the signing secret shown when you create the webhook in the AgentPhone dashboard. If empty, HMAC verification is skipped and a warning is logged — not recommended for production."},
            {"name": "AGENTPHONE_PORT", "prompt": "Local port for the webhook listener (default 8646)", "password": False,
             "help": "Port Hermes binds for POST /agentphone/webhook. Expose this port publicly (behind TLS) so AgentPhone can reach it."},
            {"name": "AGENTPHONE_VOICE", "prompt": "Default TTS voice (e.g. Polly.Amy, Polly.Joanna — leave empty for AgentPhone's default)", "password": False,
             "help": "Optional. Applies to every outbound call unless overridden per-call via send_message(..., voice=...)."},
        ],
    },
]
